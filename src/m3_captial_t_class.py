...
Authors: David Mutchler, Dave Fisher, Valerie Galluzzi, Amanda Stouder,
         their colleagues and Nathaniel N Nordquist, Nate.
...
class CapitalT(object):
    ...
    def __init__(self, intersection_center, width, height, letter_thickness):
        ...
        self.top_corner = rg.Point(intersection_center.x - .5*self.width, intersection_center.y + .5*self.thickness)
        self.bottom_corner = rg.Point(intersection_center.x + .5*self.width, intersection_center.y - .5*self.thickness)
        self.upper_corner = rg.Point(intersection_center.x - .5*letter_thickness, intersection_center.y +
                                 .5*letter_thickness)
        self.lower_corner = rg.Point(intersection_center.x + .5*letter_thickness, intersection_center.y + height -
                                .5*letter_thickness)
        self.h_rect = rg.Rectangle(self.top_corner, self.bottom_corner)
        self.v_rect = rg.Rectangle(self.upper_corner, self.lower_corner)
    ...
    def move_by(self, dx, dy):
        ...
        # the move_by method already exists in rosegraphics. It moves the corners. I was trying to implement it by
        # moving the center.
        # self.corner_1.x += dx
        # self.corner_1.y += dy
        # self.corner_2.x += dx
